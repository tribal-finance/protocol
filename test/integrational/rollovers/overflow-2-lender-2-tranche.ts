...
describe("Rollovers (2 Lender / 2 Tranche) ensure pool can't stretch beyond max capacities", function () {
  context("For unitranche pool", async function () {
    ...
    it("borrowerExcessSpread() is 1791", async () => {
      expect(await lendingPool.borrowerExcessSpread()).to.equal(USDC(1791));
    });
    ...
    describe("test out rolling over into next pool", async () => {
      ...
      it("expect the $2000 from lender1 was rolled over but not the captial from lender2", async () => {
        expect(await firstTrancheVault.balanceOf(await lender2.getAddress())).equals(USDC(1600))
        expect(await secondTrancheVault.balanceOf(await lender2.getAddress())).equals(USDC(400))
        await nextLendingPool.executeRollover(lendingPool.address, [firstTrancheVault.address, secondTrancheVault.address]);
        expect(await firstTrancheVault.balanceOf(await lender2.getAddress())).equals(USDC(0))
        expect(await secondTrancheVault.balanceOf(await lender2.getAddress())).equals(USDC(0))
