...
npx hardhat encode-pool-initializer \
  ...
  --lending-term-seconds 45 \
  --first-loss-assets 200 \
  --repayment-recurrence-days 30 \
  --grace-period-days 5 \
  --borrower-total-interest-rate-wad 100 \
  --borrower-address $BORROWER_ADDRESS \
  --protocol-fee-wad 0.1 \
  --default-penalty 0.5 \
  --penalty-rate-wad 0.02 \
  --tranches-count 3 \
  --tranche-a-p-rs-wads "0.1,0.1,0.2" \
  --tranche-boosted-a-p-rs-wads "0.1,0.1,0.2" \
  --tranche-boost-ratios "2,2,3" \
  --tranche-coverages-wads "1,1,1" \
  --funding-split-wads "0.7,0.7:0.2,0.2:0.1,0.1"
